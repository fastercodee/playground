...
export function useVerifyOAuth2() {
  ...
  const id_token = computed(() => {
    return (route.hash ? new URLSearchParams(route.hash.slice(1)).get("id_token") : undefined) ??
    route.query.type === 'google' ?
    route.query.id_token :
     route.query.type === 'github' ?
     route.query.code : undefined as string
  })
