  ...
  <q-page class="flex flex-nowrap w-full w-[100vw] min-h-[100vh]">
    <q-header class="bg-[#1C2333]">
      ...
      <q-card flat class="w-full max-w-280px mx-auto mx-5 py-20 transparent">
        ...
        <q-card-section>
          <form @submit.prevent="logIn" class="w-full h-full" :disabled="verifyOAuth2 && !verifyOAuth2Failure ? '' : undefined">
            <q-input
              standout
              color="green-5"
        ...
        <q-card-section>
          <q-btn class="w-full bg-[#2B3245]" no-caps @click="loginWithGoogle" :disable="verifingOauth2"
          >
            <Icon icon="logos:google-icon" class="mr-1" />
            Continue with Google

            <q-spinner-tail v-if="route.query.type === 'google' && verifingOauth2" class='ml-1' size="20px" color="white" />
          </q-btn>
          <q-btn class="w-full bg-[#2B3245] mt-3" no-caps @click="loginWithGithub" :disable="verifingOauth2"
          >
            <Icon icon="logos:github-icon" class="mr-1" />
            Continue with Github

            <q-spinner-tail v-if="route.query.type === 'github' && verifingOauth2" class='ml-1' size="20px" color="white" />
          </q-btn>
        </q-card-section>
...
import { loginWithGoogle, loginWithGithub } from "boot/auth"
